...
const AnalysisPanel: React.FC<AnalysisPanelProps> = ({ entries, onAnalysisDone }) => {
  ...
  const handleAnalyze = async () => {
    ...
    setLoading(true);
    try {
      // Runs local pattern matching logic - 100% private, no API required
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 mb-10">
          <div className="max-w-xl">
            ...
            <p className="text-slate-400 text-base leading-relaxed">
              Our secure matching engine finds correlations in your logs without ever leaving your device.
              <span className="text-indigo-400 font-medium ml-1">No account or setup required.</span>
            </p>
          </div>
          ...
          <button
            ...
            className="group bg-white hover:bg-slate-50 disabled:bg-slate-800 text-slate-900 font-black py-4 px-12 rounded-2xl transition-all shadow-xl shadow-white/5 flex items-center justify-center space-x-3 active:scale-95 disabled:opacity-50 h-16 min-w-[240px]"
          >
            {loading ? (
              <>
                ...
                <span className="text-indigo-600">Analyzing Locally...</span>
              </>
            ) : (
              <>
                <span className="text-lg">Reveal Insights</span>
          ...
          <div className="bg-white/5 border border-white/10 rounded-3xl p-12 text-center backdrop-blur-sm">
            {entries.length < 3 ? (
              <div className="max-w-xs mx-auto">
                ...
                <p className="text-slate-500 text-sm">Log <span className="text-white">at least 3 entries</span> to enable the local pattern matching engine.</p>
              ...
              <div className="max-w-xs mx-auto">
                ...
                <p className="text-slate-300 font-bold mb-2">History Processed</p>
                <p className="text-slate-500 text-sm">Your logs are ready for trend analysis. Click 'Reveal Insights' to continue.</p>
              </div>
            )}
          </div>
        ) : (
              ...
              <div className="bg-white/5 border border-white/10 rounded-3xl p-8 backdrop-blur-md">
                ...
                <div className="flex flex-wrap gap-3">
                  {analysis.commonTriggers.length > 0 ? (
                    ...
                    <span className="text-slate-500 text-sm italic font-medium">Add more details to detect triggers.</span>
                  )}
                </div>
              </div>
              ...
              <div className="md:col-span-2 bg-emerald-500/10 border border-emerald-500/20 rounded-3xl p-8">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  ...
                  <div className="bg-slate-800/50 p-4 rounded-2xl border border-white/5 min-w-[140px] text-center">
                    <div className="text-[10px] font-bold text-slate-500 uppercase mb-1 tracking-widest">Reliability</div>
